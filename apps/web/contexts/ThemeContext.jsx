...
export function ThemeProvider({ children }) {
  ...
  const updateDocumentTheme = (dark, accent) => {
    console.log("ThemeContext - updateDocumentTheme called with:", {
      dark,
      accent,
    });
  ...
  const toggleTheme = async () => {
    ...
    const updateLocalSettings =
      useSettings.getState().updateFocusSettings || (() => {});
    ...
    if (user && userId && typeof userId === "string") {
      try {
        ...
        console.error(
          "ThemeContext - toggleTheme - server sync failed (continuing with local change):",
          error
        );
      ...
      console.log(
        "ThemeContext - toggleTheme - no authenticated user, saving to localStorage only"
      );
    }
  ...
  const changeAccentColor = async (newAccentColor) => {
    console.log(
      "ThemeContext - changeAccentColor called with:",
      newAccentColor
    );
