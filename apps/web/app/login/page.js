    ...
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full ${Icon ? 'pl-10' : 'pl-4'} ${showPasswordToggle ? 'pr-10' : 'pr-4'} py-3 border-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-base min-h-12 ${
            error 
              ? isDark 
              ...
              : isDark 
                ? 'border-gray-600 bg-gray-800/50 text-gray-100 placeholder-gray-400 hover:border-gray-500 active:border-gray-400' 
                : 'border-gray-200 bg-white text-gray-900 placeholder-gray-500 hover:border-gray-300 active:border-blue-400'
          }`}
        />
        {showPasswordToggle && (
          <button
...
function GoogleLoginButton({ onGoogleSignIn, isLoading }) {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className={`w-full flex items-center justify-center gap-3 border-2 py-4 px-4 rounded-xl font-medium focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 h-12 ${
        isDark 
          ? 'bg-gray-800 border-gray-600 hover:bg-gray-700 hover:border-gray-500 text-gray-200 active:bg-gray-600' 
          : 'bg-white border-gray-200 hover:bg-gray-50 hover:border-gray-300 text-gray-900 active:bg-gray-100'
      }`}
    >
      {isLoading ? (
        <>
...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      window.location.href = '/tasks'; // Full reload (WebView safe)
    } catch (error) {
      console.error("Google sign in failed:", error);
      setRedirecting(false);
      setRedirectAttempts(0);
      // Error is already set by googleSignIn in AuthContext
    } finally {
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'bg-gray-950' : 'bg-linear-to-br from-blue-50 via-white to-purple-50'} flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-md w-full">
        {/* Header */}
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-linear-to-r from-blue-600 to-purple-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <Image
            src="/icon.svg"
            alt="App Icon"
            width={64}
          ...
          <h1 className={`text-3xl font-bold mb-2 ${
            ...
            Welcome to <span className="text-transparent bg-clip-text bg-linear-to-r from-blue-600 to-purple-600">iTaskOrg</span>
          </h1>
