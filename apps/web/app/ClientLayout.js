...
function UserProfile({ user }) {
  ...
  return (
    ...
    >
      <div className="w-10 h-10 rounded-full bg-linear-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white overflow-hidden">
        {avatarContent}
      </div>
      ...
      <div className="flex items-center justify-between p-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/profile" className="flex items-center">
            <div className="w-8 h-8 rounded-full bg-linear-to-r from-blue-600 to-purple-600 flex items-center justify-center text-white overflow-hidden">
              {profile.avatar ? (
                <Image
                  src={profile.avatar}
                  alt={displayName}
...
function MobileNavOverlay({ isOpen, onClose, user }) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Close UI immediately (safe)
      onClose?.();

      // 🔥 HARD redirect – middleware-safe
  ...
  return (
    <>
      ...
      <nav
        ...
        } border-r shadow-2xl z-50 transform transition-transform duration-300 ${
      ...
      >
        ...
        >
          <h1 className="text-2xl font-bold text-transparent bg-clip-text bg-linear-to-r from-blue-400 to-purple-400">
            iTaskOrg
          </h1>
          <button
            onClick={onClose}
        ...
        >
          <button
            ...
            className={`flex items-center space-x-3 p-3 w-full rounded-xl ${
              isDark
  ...
  return (
    <nav
      className={`hidden lg:flex w-64 ${
        isDark ? "bg-gray-950 border-gray-800" : "bg-white border-gray-200"
      } border-r shadow-sm fixed h-screen overflow-y-auto flex-col scrollbar-hide z-30`}
    >
      <div
        className={`p-4 border-b ${
          isDark ? "border-gray-800" : "border-gray-200"
      ...
      >
        <button
          onClick={handleLogout}
          className={`flex items-center space-x-2 p-3 w-full rounded-xl ${
            isDark
              ? "text-red-400 hover:bg-red-900/30"
              : "text-red-600 hover:bg-red-50"
          } transition-colors`}
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-gray-950" : "bg-gray-50"}`}>
      ...
      {process.env.NODE_ENV === "development" &&
        (() => {
          const TaskDebugger =
            require("../components/debug/TaskDebugger").default;
          return <TaskDebugger />;
        })()}
    </div>
  );
